...
window.ReportsPresupuestoView = {
    ...
    async loadData() {
        try {
            const [items, branches, asignaciones] = await Promise.all([
    ...
    renderPrintView(data) {
        ...
        if (this.config.rubroId !== '') {
            ...
            totalInicial = parentItem ? parseFloat(parentItem.valor_total_oficial) : 0;
        } else {
            totalInicial = data.reduce((acc, item) => !item.padre_id ? acc + (parseFloat(item.valor_total_oficial) || 0) : acc, 0);
        }
        ...
        data.forEach(item => {
            ...
            // For now, initially budget reports has 0 credit operations
            const creditos = 0;
            const ccreditos = 0;
            const definitivo = item.valor_total_oficial + creditos - ccreditos;

            tableHtml += `
                <tr class="${rowClass}">
                    <td>${item.codigo}</td>
                    <td style="padding-left: ${8 + indent}px !important;" class="text-uppercase">${item.nombre}</td>
                    <td class="text-end">${Helper.formatCurrency(item.valor_total_oficial)}</td>
                ...
                item.distribucion.forEach(d => {
                    ...
                    tableHtml += `
                        <tr>
                            ...
                            <td class="text-end text-muted">${Helper.formatCurrency(d.valor_inicial)}</td>
                            <td class="text-end text-success text-muted">$0.00</td>
                            <td class="text-end text-danger text-muted">$0.00</td>
                            <td class="text-end text-muted">${Helper.formatCurrency(d.valor_inicial)}</td>
                        </tr>
                    `;
                });
...
<div class="bg-white p-4 min-vh-100">
    ...
    <div class="text-center mb-4 pb-2 border-bottom">
        <h4 class="fw-bold text-uppercase mb-1">PRESUPUESTO INICIAL DE INGRESOS Y GASTOS</h4>
        <small class="text-muted fw-bold">Fecha Generación: ${dateStr}</small>
    </div>
...
`;
    ...
    exportExcel() {
        ...
        let html = `
            ...
            <body>
                <h2 style="text-align: center;">PRESUPUESTO INICIAL DE INGRESOS Y GASTOS</h2>
                <table>
                    ${table.innerHTML}
                </table>
            </body>
