...
file_put_contents($file, $users_js_content);
echo "SUCCESS: users.js fixed with render() method.\n";

// Bump version to 1.2.0
$config_js = 'app/assets/js/core/config.js';
$c = file_get_contents($config_js);
$c = preg_replace("/VERSION: '.*'/", "VERSION: '1.2.0'", $c);
file_put_contents($config_js, $c);
echo "SUCCESS: config.js bumped to 1.2.0.\n";

$app_index = 'app/index.php';
$ai = file_get_contents($app_index);
$ai = preg_replace("/\\\$version = \".*\"/", "\$version = \"1.2.0\"", $ai);
file_put_contents($app_index, $ai);
echo "SUCCESS: app/index.php bumped to 1.2.0.\n";
